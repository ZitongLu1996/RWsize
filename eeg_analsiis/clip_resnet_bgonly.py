...
root='../images/test_images/forloading_backgroundonly'
batch_size = 32

dataset = ImageDataset(
  root=root,
  out_path='clip_resnet_features_bgonly',
...
save_features(features, out_path='clip_resnet_late_features_bgonly', file_format='npy')

import numpy as np

features = np.load('clip_resnet_late_features_bgonly/features.npy')
print(features)
...
np.save('ann_rdms/clip_resnet_late_bgonly.npy', rdm_dnn)
...
save_features(features, out_path='clip_resnet_early_features_bgonly', file_format='npy')

import numpy as np

features = np.load('clip_resnet_early_features_bgonly/features.npy')
print(features)
...
np.save('ann_rdms/clip_resnet_early_bgonly.npy', rdm_dnn)
